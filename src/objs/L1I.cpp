static const char rcs_id_l1i_c[] =
  "@(#) $Id: L1I.cpp,v 11.11 2013/04/11 20:57:40 cveerama Exp $";
...
L1I::allocateArrays()
{
  ...
  num_hs_bins=(int**)make_array(sizeof(int),2,
					 L1I_RANGE_WIDTH,L1I_DOPPLER_WIDTH);
  bytes+=size_array(sizeof(int),2,L1I_RANGE_WIDTH,L1I_DOPPLER_WIDTH);

  num_hs_bins_amb=(int**)make_array(sizeof(int),2,
					 L1I_RANGE_WIDTH,L1I_DOPPLER_WIDTH);
  bytes+=size_array(sizeof(int),2,L1I_RANGE_WIDTH,L1I_DOPPLER_WIDTH);
...
L1I::~L1I(){
  ...
  free_array((void*)num_hs_bins,2,L1I_RANGE_WIDTH,L1I_DOPPLER_WIDTH); 
  free_array((void*)num_hs_bins_amb,2,L1I_RANGE_WIDTH,L1I_DOPPLER_WIDTH); 
  ...
  if(s0frombidr!=NULL){
    free_array((void*)s0frombidr,2,L1I_RANGE_WIDTH,L1I_DOPPLER_WIDTH); 
    s0frombidr=NULL;
  }
...
void L1I::writeHeader(int num_records) 
  ...
  if ( header.length() != header_size_){
    ...
    sprintf(msg,"Incorrect header size (%d bytes) should be %d",
	    (int)header.length(),(int)header_size_);
...
void L1I::computeWindows()
{
  ...
  // BUG FIX 08/20/2012 BWS because range compression reduces the width of the echo by the chirplength the echo will NOT be 
  // centered in the range bins, this was screwing up HISAR calibration and dopper and range centroid estimation for beams 1-2 and 4-5
  // Luckily this things were only done for beam 3 where there is little effect
  ...
  //-------- new version 
  if(pulse_spread_in_pri!=1.0){
    fprintf(stderr,"The PULSE_SPREADING_IN_PRI keyword is NOT 1. This option is obsolete.");
    exit(1);
  }
  start_range_bin=0;
  end_range_bin=start_range_bin+(int)pulse_spread_in_pri*window_step/2-1;
  //---- end BUG FIX
...
// Enceladus SAR
L1I::HISARcalibrate(SARProcParams& spp, double radii[3], double  freq_shift, int ambigmode, double tol=0.1){
  ...
  // 5) ambigmode=4 is same as 1 except Xfactor is set to 0 when ambtosig ratio exceeds 0.2, of Xfcator is more than 10 dB below its 
  //    max value of we are outside the processing window. (Used by sartopo_calibrate)
  // 6) ambigmode=5 is same as 4 except ambtosig ratio and low Xfactor cases are not marked invalid, this is used for all clls by 
  // sar_topo except the first which is ambigmode=5
  //---------------------------
  ...
  // set up option for zeroing out Xfactor where needed by ambigmode=4 which is called by sartopo_calibrate
  bool sartopo_calib=false; // A kludge-- I know --- sigh
  bool sartopo_init=true;
  if(ambigmode==4){
    ambigmode=1;
    sartopo_calib=true;
  }
  ...
  else if(strcasecmp("Dione",default_target_name.c_str())==0){
    backscat_mod_type=DIONE_WYE1;
  }
  else{
  ...
  for(int a=0;a<Nazim;a++){
	...
	float s0=theorBackscatter(inc,backscat_mod_type);
      ...
      //---------------------
       ...
       float relrange=(r+rc_range_shift_in_km-range_min)/range_step_; // includes estimated range centroid shift range_shift_in_km 
                                                                      // strictly speaking this shift should be in gain only rather than X
                                                                      // for consistency with calibrate()
	 ...
	 inprocwindow=false;
       ...
       float jf= (reldop+ac_dop_shift_in_Hz-mindop)/doppler_step_; // includes estimated Doppler centroid shift term ac_dop_shift_in_Hz
  ...
  //-----------------------
  // smooth out errors due to large integration step size
  // by adjusting for variation in number of integration bins per pixel
  //-----------------------
  for(unsigned int i=0;i<num_range_bins;i++){
    for(int j=0;j<Nfft_az;j++){
      int nneigh=0, nneigh_amb=0;
      float ave_hs_bin=0,ave_hs_bin_amb=0;
      for(int i2=(int)i-5;i2<(int)i+5;i2++){
	for(int j2=j-5;j2<j+5;j2++){
	  if(i2<0 || i2>=(int)num_range_bins) continue;
	  if(j2<0 || j2>=Nfft_az) continue;
	  if(num_hs_bins[i2][j2]!=0){
	    ave_hs_bin+=num_hs_bins[i2][j2];
	    nneigh++;
	  }
	  if(num_hs_bins_amb[i2][j2]!=0){
	    ave_hs_bin_amb+=num_hs_bins_amb[i2][j2];
	    nneigh_amb++;
	  }
	} // end j2 loop
      } // end i2 loop
      if(nneigh>0) ave_hs_bin/=nneigh;
      if(nneigh_amb>0) ave_hs_bin_amb/=nneigh_amb;
      if(num_hs_bins[i][j]!=0){
	Xfactor[i][j]*=ave_hs_bin/num_hs_bins[i][j];
	ests0[i][j]*=ave_hs_bin/num_hs_bins[i][j];	
      }
      if(num_hs_bins_amb[i][j]!=0){
	Xambig[i][j]*=ave_hs_bin_amb/num_hs_bins_amb[i][j];
	ests0ambig[i][j]*=ave_hs_bin_amb/num_hs_bins_amb[i][j];	
      }
    } // end j loop
  }  // end i loop
  //--------------------------
  ...
  // ambigmode 4-5 kludge used by sartopo_calibrate()
  if(sartopo_calib){
    // zero out data outside processing window or greater than 20% ambig ratio
    // or less than 10 dB down from peak
    float maxX=0;
    float ambratio=0.20;
    for(unsigned int i=0;i<num_range_bins;i++){
      for(int j=0;j<Nfft_az;j++){
	if((int)i<start_range_bin || (int)i>end_range_bin) Xfactor[i][j]=0;     
	if(sartopo_init && Xambig[i][j]>(ambratio/(1+ambratio))*Xfactor[i][j]) Xfactor[i][j]=0;
	if(sartopo_init && Xfactor[i][j]>maxX) maxX=Xfactor[i][j];
      }   
    }
    for(unsigned int i=0;i<num_range_bins;i++){
      for(int j=0;j<Nfft_az;j++){
	if(Xfactor[i][j]<0.1*maxX) Xfactor[i][j]=0;
      }
    }
  ...
  } // end of special option sartopo_calib
...
float L1I::XAmbigInterpolate(){
  // get each weight using  SincTable::weight()
  // and compute weighted sum
  float X;
  if (! use_sinc_interpolation_)
  {
    X=Xambig[current_range_idx_][current_doppler_idx_];
  }
  else
  {
    // INCOMPLETE -- still uses nearest neighbor
...
// Real AP SARTopo Routines INCOMPLETE
void
L1I::getBIDRSigma0s(BIDRFile& bf){
  float corrected_range,range_correction;
  double slat,slon,lat,lon;
  int ppd=bf.pixelsPerDegree();
  int absmaxnumline=ppd*360;
  int ngoodsigma0=0;
  int line,sample;
   for(unsigned int i=0;i<num_range_bins;i++){
     for(int j=0;j<Nfft_az;j++){   

       range_correction=absDoppler(i,j)-nominal_doppler_centroid_in_Hz;
       range_correction*=range_doppler_correction_coeff;
       corrected_range=(float)(range[i]+range_correction);

       bool onbody=dopplerRangeHeightToLatLon(absDoppler(i,j),corrected_range,0,slat,slon);
       if(onbody){
	 while (slon < 0) { slon += 2*pi; }
	 while (slon >= 2*pi) { slon -= 2*pi; } 
	 lat = bf.latInRad(slon, slat);
	 lon = bf.lonInRad(slon, slat);
	 line = (int)(bf.OCLonToGrid(lon)+0.5);
	 while(line>absmaxnumline) line-=absmaxnumline;
	 sample = (int)(bf.OCLatToGrid(lat)+0.5);  
	 
	 if(line>0 && line<= bf.numLons() && sample>0 && sample<=bf.numLats()){
	   int offset = sizeof(float)*(sample-1) + (line-1)*bf.numLats()*sizeof(float); 
	   float value;
	   offset+=bf.labelLength();
	   bf.setPosition(offset);
	   bf.read(value);
	   s0frombidr[i][j]=value;
           if(s0frombidr[i][j]<-1e30){
	     s0frombidr[i][j]=0; // invalid s0 value case 
	   }
	   else{
	     ngoodsigma0++;
             means0+=value;
...
void
L1I::realap_sartopo_calibrate(SARProcParams& spp, ofstream& afs, float h){
  double trad[3];
  for(int c=0;c<3;c++) trad[c]=default_target_radii_in_km[c]+h;
  HISARcalibrate(spp,trad,0,1,0.1);
}  

void
L1I::sartopo_calibrate(SARProcParams& spp, ofstream& afs, float h, bool init_mode){
  double trad[3];
  for(int c=0;c<3;c++) trad[c]=default_target_radii_in_km[c]+h;
  int ambigmode=4;
  if(!init_mode) ambigmode=5;
  HISARcalibrate(spp,trad,0,ambigmode,0.1); // see ambig_mode=4 comment in HISARcalibrate

}  

float 
L1I::getPowerRMSError(SARProcParams& spp,float h=0,bool dbg=false){
  float err=0;
  float n=(end_range_bin+1-start_range_bin)*Nfft_az;
  float Xtot=0;
  float Xambigtot=0;
  ...
  FILE* tfp;
  if(dbg){
    sprintf(ofile,"debug_%d_%g.txt",sab_counter,h);
    tfp=fopen(ofile,"w");

    fwrite(&num_range_bins,sizeof(int),1,tfp);
    fwrite(&Nfft_az,sizeof(int),1,tfp);
    fwrite(&beam_number,sizeof(int),1,tfp);
    fwrite(&(spp.quant_noise_scale),sizeof(float),1,tfp);
    fwrite(&(spp.quant_noise_offset),sizeof(float),1,tfp);
    fwrite(&(spp.thermal_noise_offset),sizeof(float),1,tfp);
  }
  for(unsigned int i=0;i<num_range_bins;i++){
    for(int j=0;j<Nfft_az;j++){ 
      Xambigtot=Xambigtot+Xambig[i][j];
      Xtot=Xtot+Xfactor[i][j];

      // target is actual detected raw SAR power
      float y=abs(raw_sar_data[i][j]);
      y=y*y;

      // estimate of raw power from geometry, best sigma0, and noise terms (presumes Xambig was added into Xfactor in HISARcalibrate)
      float x=(s0frombidr[i][j]*(Xfactor[i][j])+spp.thermal_noise_offset)*spp.quant_noise_scale+spp.quant_noise_offset;
      // add to error
      if(dbg){
	fwrite(&x,sizeof(float),1,tfp);
	fwrite(&y,sizeof(float),1,tfp);
	fwrite(&Xfactor[i][j],sizeof(float),1,tfp);
	fwrite(&Xambig[i][j],sizeof(float),1,tfp);
	fwrite(&s0frombidr[i][j],sizeof(float),1,tfp);
      }     
      if(!isfinite(x) | !isfinite(y)){
	fprintf(stderr,"Nonfinite x or y term in getPowerRMSError...\nBug report follows\n");
	fprintf(stderr,"i=%d of %d, j=%d of %d\ny=%g x=%g s0frombidr=%g xfactor=%g Xambig=%g\n",i,num_range_bins,j,Nfft_az,y,x,s0frombidr[i][j],Xfactor[i][j],Xambig[i][j]);
        exit(1);
      }
      if((int)i>=start_range_bin & (int)i<=end_range_bin){
 	meanx=meanx+x;
 	meany=meany+y;
 	stdx=stdx+x*x;
 	stdy=stdy+y*y;
  ...
  meanx=meanx/n;
  meany=meany/n;
  stdx=sqrt((stdx-meanx*meanx*n)/(n-1));
  stdy=sqrt((stdy-meany*meany*n)/(n-1));
  ...
  for(unsigned int i=0;i<num_range_bins;i++){
    for(int j=0;j<Nfft_az;j++){
      ...
      float x=(s0frombidr[i][j]*(Xfactor[i][j])+spp.thermal_noise_offset)*spp.quant_noise_scale+spp.quant_noise_offset;
      // add to error
      float e=(x-meanx)/stdx-(y-meany)/stdy;

      if(!isfinite(e)){
	fprintf(stderr,"Nonfinite error term in getPowerRMSError...\nBug report follows\n");
	fprintf(stderr,"i=%d of %d, j=%d of %d\ny=%g x=%g s0frombidr=%g xfactor=%g Xambig=%g\n",i,num_range_bins,j,Nfft_az,y,x,s0frombidr[i][j],Xfactor[i][j],Xambig[i][j]);
        exit(1);
      }
      if((int)i>=start_range_bin & (int)i<=end_range_bin) err=err+e*e;
    }
  }
  err=sqrt(err/n);
  ambigfrac=Xambigtot/Xtot;
  if(dbg) fclose(tfp);     
  return(err);
}

void 
L1I::allocateRealAppSartopoArrays(){
  s0frombidr=(float**)make_array(sizeof(float),2,
					 L1I_RANGE_WIDTH,L1I_DOPPLER_WIDTH);
  if(s0frombidr==NULL){
    fprintf(stderr,"L1I error allocating s0frombidr\n");
    exit(1);
  }
}
