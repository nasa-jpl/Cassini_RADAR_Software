...
//------------------------------------------------
//spline interpolation of (x1,y1) w.r.t a new basis x2
//----------------------------------------------
void spline_interpolation(const vector<Uvar>& x1,
  ...
  if(N!=y1.size()) ErrorMessage("spline.cpp: (x1,y1) size mismatch").throwMe();
  ...
  for(unsigned int i=0;i<x2.size();++i){
    yval=spline_cubic_val((int)N, t , x2[i].getValue(), y,ypp, &ypval, &yppval);
    y2.push_back(yval);
  }
  
  if(y2.size()!=x2.size()) ErrorMessage("(x2,y2) size mismatch").throwMe();
  delete[] ypp;
  return;
}
...
void spline_interpolation(const vector<double>& x1,
  ...
  for(unsigned int i=0;i<x2.size();++i){
    yval=spline_cubic_val((int)N, t , x2[i], y,ypp, &ypval, &yppval);
    y2.push_back(yval);
  }
