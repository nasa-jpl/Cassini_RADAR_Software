...
#define LBDR_RECORD_LENGTH 132344
...
int main(int argc, char* argv[])
{
  ...
  //-------------------
  float*  radar_data;
  radar_data = new float[Nradar_data_max];
  for(unsigned int i=0;i<Nradar_data_max;++i) radar_data[i]=0.0;
  ...
  //----------
  if(fseek(fp,LBDR_RECORD_LENGTH,0)!=0){
  ...
  // and record length
  //------------------------
  from_sab_counter_to_end= LBDR_RECORD_LENGTH - from_top_to_sab_counter -sizeof(unsigned int);
  cout<<"Num Records  "<<Nrecord<<endl;
  cout<<"Record length "<<LBDR_RECORD_LENGTH<<endl;
  ...
  for(unsigned int i=0;i<Nrecord;++i){
    ...
    // record legnth: LBDR_RECORD_LENGTH
    //--------------------------------
    f_position=ftell(fp);
    ...
    cout<<"delvyr "<<delvyr<<endl;
    cout<<"delvmn "<<delvmn<<endl;
    cout<<"delvda "<<delvda<<endl;
    ...
    cout<<"sc position "<< sc_position_target_x<<" "<< sc_position_target_y<<" "<< sc_position_target_z<<endl;
    cout<<"sc v "<< sc_velocity_target_x<<" "<< sc_velocity_target_y<<" "<< sc_velocity_target_z<<endl;
    ...
    cout<<"sigma0_uncorrected "<< sigma0_uncorrected << endl;
    ...
    for(unsigned int j=0;j<Nradar_data;++j){
      readFloat(radar_data[j],byte_flip,fp);
    //  cout<<j<<"th sample value:  "<< radar_data[j]<<endl;
    }
    cout<<"first value:  "<< radar_data[0]<<endl;
    cout<<"last value: "<< radar_data[Nradar_data-1]<<endl;
    f_position=ftell(fp);
    f_position+=sizeof(float)*(Nradar_data_max - Nradar_data);
    if(fseek(fp,f_position,0)!=0){
      cerr<<"Error setting current file position "<<endl;
      exit(-1);
    }
    ...
    if( (end_position- start_position)!=LBDR_RECORD_LENGTH){
