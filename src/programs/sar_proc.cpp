...
try
  ...
  //------------------------  
    ...
    Config cfg("sar_proc",config_file);
    ...
    BIDR bidr(bidr_prefix,cfg,l1b,spp,"w",NORMAL); 
    // Class for manipulating BIDR files 
    // handles beam mask, incidence angle, and backscatter
    // images simultaneously
    // set up BIDR projection and write BIDR headers

    bidr.writeHeaders(); 

    // set up Doppler and Range Output file if keyword exists
    RangeDopplerFile* rdfp=NULL;
    if(cfg.keywordExists("SAR_PROC_RANGE_DOPPLER_OUTPUT_FILE")){
      string fname = cfg.str("SAR_PROC_RANGE_DOPPLER_OUTPUT_FILE");
      rdfp=new RangeDopplerFile(fname,"w");
      cerr << "Warning: Special Mode Outputting Range/Doppler info to " 
           << fname << endl;
    }
    ...
    afs.precision(5);
    afs << "!! Cassini RADAR Sar Processor Version 0.1" << endl;
	...
	continue;
      ...
      // write Range Doppler info if desired
      if(rdfp){
	  l1i.outputRangeDoppler(*rdfp);
	}
	...
	bidr.stripProcessAndWrite(l1i,spp,single_burst_mode); 
      ...
      // compute and writes SAR ancillary data to L1B input/output file
      // L1B::outputSARAncillaryData()
      // assumes whole record has already been read.
      // Jostles file pointer around an eventally puts it back
      // to the end of the record. (L1B routine)
      l1i.computeSARAncillaryData(spp,bidr.getProjection());
      l1b.outputSARAncillaryData(l1i); 
   ...
   // delete RangeDopplerFile object if necessary
   if(rdfp) delete rdfp;
